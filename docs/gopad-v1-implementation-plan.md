...
Verification so far: `go test ./...`, `go vet ./...`, browser tests, YAML/JSON parsing, and static logo validation pass. Docker is unavailable on the local Windows host. The first GitHub Actions Docker smoke-test attempt failed before `/healthz` responded; the image now initializes the persistent `/data` volume for its non-root user. A later run passed the application, metrics, and Prometheus checks but Grafana reset the connection while starting; the workflow now retries all transient errors for the Grafana health probe, and still prints Compose logs on failure.
